...
interface Address {
  id: string; fullName: string; line1: string; city: string; isDefault: boolean;
}
...
export default function CheckoutPage() {
  ...
  // New-address form
  const [form, setForm] = useState({ fullName: '', phone: '', line1: '', city: '', postalCode: '' });

  const loadAddresses = () =>
    get('/api/addresses').then((a: Address[]) => {
      setAddresses(a);
      const match = a.find((x) => x.isDefault) || a[0];
      if (match) setSelected(match.id);
    });

  useEffect(() => { loadAddresses().catch((e) => setErr(e.message)); }, []);

  async function addAddress() {
    setErr('');
    try {
      const a = await post('/api/addresses', form);
      await loadAddresses();
      setSelected(a.id);
      setForm({ fullName: '', phone: '', line1: '', city: '', postalCode: '' });
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      <Card className="mb-4">
        ...
        <CardContent className="space-y-3">
          {addresses.map((a) => (
            <label key={a.id} className="flex items-center gap-3 rounded-md border p-3 cursor-pointer">
              ...
              <span className="text-sm">
                <strong>{a.fullName}</strong> — {a.line1}, {a.city}
                {a.isDefault && ' (default)'}
              </span>
            </label>
          ))}
          {addresses.length === 0 && (
            <p className="text-sm text-muted-foreground">No address yet — add one below.</p>
          )}
          ...
          <div className="grid grid-cols-2 gap-2 pt-2">
            ...
            <Input placeholder="Pincode" value={form.postalCode} onChange={(e) => setForm({ ...form, postalCode: e.target.value })} />
          </div>
      ...
      <div className="flex items-center justify-between mb-4">
        <span>Subtotal</span>
        <span className="font-medium">{money(cart?.subtotalMinor ?? 0, cart?.currency ?? 'inr')}</span>
      </div>
